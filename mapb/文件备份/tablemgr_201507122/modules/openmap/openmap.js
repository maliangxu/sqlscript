...
define(function (require, exports, module){ 
	...
	_selMapHandler: function (clickImg){
		...
		var sharetype = parseInt(clickImg.getAttribute('sharetype')) || 4;
	...
	confirmExec : function (){
		...
		this.requestTbs(resid, uid, canshu, this.succCallback);
		this.hidediv();
	},

	requestTbs: function (dbid, uid, canshu, callback){
		//var usrname = this.userId;
		var path = '';
		if(canshu == 'me'){
			path = "mapb.tablemgr.openmap.requestTb";
		}else{
			path = "mapb.tablemgr.openmap.requestOtherTb";
		}
...
/*	$('#accessMapList img').hover (function(evt) { console.log('hover');
	...
	$('#accessMapList').on('dblclick','img',function (evt){
		var clickImg = evt.target || evt.srcElement;
		openmapMod.resrealname = $(this).next().text();
		if(clickImg.tagName=='IMG'){
			$('#accessMapList img').each(function (){ 
