--复制分享:先将地图工作空间复制一份，复制要素表，并更新资源表
get_db mapb,sysdb
...
--向资源表中插入记录(地图复制为该用户的资源)
select 'respath='||'mapb/'||:new_mapid as sql_add_param

--select 'time='|| datetime('now','localtime') as sql_add_param

insert or replace into sysdb_resources(flag,resid,restype,resowner,respath,resdesc,imgurl,keywords,visitcount,[timestamp])
select 0,:new_mapid,'map',:userid,:respath,:description,imgurl,keywords,0,:timer from sysdb_resources r where r.resid=:mapid and restype='map' and r.flag>=0
