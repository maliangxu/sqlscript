...
create table if not exists g_layers(lyr_id integer,lyr_name text,lyr_type integer,visible integer,z_index integer,alias text,datasource text,selectbale integer,autolabel integer,zoomlayer text,lablefield text,lyr_style text,labelstyle text,lyr_flag integer default 0,primary key(lyr_id))
