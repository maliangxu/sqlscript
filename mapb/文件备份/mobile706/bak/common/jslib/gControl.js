...
gEcnu.Control.Scale = gEcnu.Control.extend({
    ...
    onAdd: function(map){
     ...
     _initContainer: function(map){
         ...
         scaleCanvas.style.backgroundColor='rgba(255,255,255,0.2)';
     ...
     _showScale: function(size){
         ...
         ctx.lineTo(width/2+left,top+3);
         ctx.lineTo(width/2+left,top);
         ctx.closePath();
         ctx.fill();
...
/*         ctx.beginPath();
     ...
     _showScaleText: function(ctx,h){
         ...
         ctx.fillText(halfkmscale, left+widthHalf-18, top + 20);
