	...
	*/
{
	...
	_initEvent: function () {
		...
		*/
		...
		if (startName in doc) {
			isTouch = true;
			gEcnu.Util.addEvtHandler(ctn,'touchstart',mapMouseDownEvt_pan);
		    gEcnu.Util.addEvtHandler(ctn,'touchmove',mapMouseMove_pan);
		    gEcnu.Util.addEvtHandler(ctn,'touchend',mapMouseUp_pan);
		}
		...
		var touchmovebool=false;//手指滑动
		var dFingersOneleave=false;//两点触摸事件，一只手指头离开
		var townfingersdown=false;//两指按下
		var singletouchdown;
		var onefingermove=false,onfingermovetowndown=false,onemovetwoxy={};//一只手指头先滑动，再出发缩放
        ...
        function mapMouseDownEvt_pan(e){
            ...
            if(gSelf.mode == 'map'&&(gSelf.mapTool != "rulerLength")&&(gSelf.mapTool != "rulerArea")){                     //  mapTool==pan时
                ...
                mapMouseDown_pan(e);
                /*document.onmousemove=moveAllEvent;
                //gEcnu.Util.addEvtHandler(document,'touchmove',moveAllEvent);
                function moveAllEvent(e){
            	    if(gSelf.forbidPan_ex){
                	    gSelf.mode=gSelf.tmpMapmode;
                        gSelf.mapTool=gSelf.tmpMaptool;
                	    return;
                ...
                document.onmouseup=upAllEvent;
                //gEcnu.Util.addEvtHandler(document,'touchend',upAllEvent);
                function upAllEvent(e){
                	if(gSelf.forbidPan_ex){
                	    gSelf.mode=gSelf.tmpMapmode;
                        gSelf.mapTool=gSelf.tmpMaptool;
                	    return;
                    ...
                    //gEcnu.Util.addEvtHandler(document,'touchmove',null);
                    //gEcnu.Util.addEvtHandler(document,'touchend',null);
                	mapMouseUp_pan(e);  
                	upAllEvent=null; 
                }*/
		...
		function mapMouseDown_pan(e) {
			...
			if(dFingersOneleave)return;
			if (gSelf.mode == 'map') {
				if (e.type == 'touchstart' && e.touches.length == 2) {
					/*var dltx = gSelf.currentX - gSelf.startScrX;
					var dlty = gSelf.currentY - gSelf.startScrY;
					...
					if(onefingermove){
						onfingermovetowndown=true;
						//onemovetwoxy.x = scrxy.x - gSelf.startScrX;
						//onemovetwoxy.y = scrxy.y - gSelf.startScrY;
					}
					...
					singletouchdown=undefined;
					townfingersdown=true;
				} else {
					...
					moveMonitor.startScrX= scrxy.x;
					moveMonitor.startScrY = scrxy.y;
					var oLayers = gSelf.getAllLayers();
					for (var i = 0; i < oLayers.length; i++) {
		...
		*/
		function mapMouseMove_pan(e) {
            ...
            if (gSelf.mode == 'map'&&gSelf.mapTool == 'pan') {
				var mxy, scrxy;
				if (e.type == 'touchmove' && e.touches.length == 1)onefingermove=true;
				if (e.type == 'touchmove' && e.touches.length == 2) {
					...
					gSelf.currentX = scrxy.x;
					gSelf.currentY = scrxy.y;
					if (gSelf.dragging && gSelf.mapTool == 'pan') {
						//var dltx = scrxy.x - gSelf.startScrX;
						//var dlty = scrxy.y - gSelf.startScrY;
						var dltx = scrxy.x - moveMonitor.startScrX;
						var dlty = scrxy.y - moveMonitor.startScrY;
						var oLayers = gSelf.getAllLayers();
						if (Math.abs(dltx) > 15 || Math.abs(dlty) > 15) {
							dltx = scrxy.x - gSelf.startScrX;
							dlty = scrxy.y - gSelf.startScrY;
						    for (var i = 0; i < oLayers.length; i++) {
						    	if (oLayers[i].visible) {
						    		oLayers[i].onDrag(dltx, dlty);
						    	}
		...
		function mapMouseMoveEvt(e) {//不使用
			gEcnu.Util.preventDefault(e);
			if (gSelf.mode == 'map') {
				var mxy;
				if (e.type == 'touchmove') {
		...
		function mapMouseUp_pan(e) {
			if (e.type == 'touchend' && dFingersOneleave){
				dFingersOneleave=false;
				townfingersdown=false;
				return;
			}
			if (e.type == 'touchend' && townfingersdown){dFingersOneleave=true;}
			if (gSelf.mode == 'map' && gSelf.mapTool == 'pan') {
				var mxy, scrxy;
				if (Math.abs(gSelf.disMinus) > 0) { //多点触摸
					if(onfingermovetowndown){
						if (Math.abs(onemovetwoxy.x) > 0 || Math.abs(onemovetwoxy.y) > 0) {
							var oLayers = gSelf.getAllLayers();
							for (var i = 0; i < oLayers.length; i++) {
								if (oLayers[i].visible) {
									oLayers[i].onDrag(-onemovetwoxy.x, -onemovetwoxy.y);
								}
							}
						}
						onfingermovetowndown=false;
						onefingermove=false;
					}
					if (gSelf.disMinus > 0) {
						gSelf.zoomIn();
					...
					mxy = gEcnu.Util.getMouseXY(gSelf._container,e);
					scrxy = gEcnu.Util.getTouchXY(e);
					if(singletouchdown&&(Math.abs(scrxy.x - singletouchdown.x)+Math.abs(scrxy.y - singletouchdown.y)<=50)&&!touchmovebool)return;
					if (gSelf.mapTool == 'pan') {
						...
						if (Math.abs(dltx) > 0 || Math.abs(dlty) > 0) {
						    ...
						    var nscx =parseInt( scxy.x )- dltx;
						    var nscy =parseInt( scxy.y )- dlty;
