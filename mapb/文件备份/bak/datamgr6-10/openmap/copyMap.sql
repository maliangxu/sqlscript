...
insert into g_map(map_name,map_type,extent,ViewExtent,coordsys,bz,map_flag)
	...
	where r.resid=:resid and restype='map' and gm.map_flag>=0
...
select lyr_name,alias,lyr_type,lyr_type,:new_mapid,ftset_id,visible,z_index,selectbale,autolabel,zoomlayer,lablefield,lyr_style,labelstyle,lyr_flag from g_layers where map_id=:resid and lyr_flag>=0

--向资源表中插入记录(地图复制为该用户的资源，但是图层不复制)
...
select 0,:new_mapid,'map',:userid,:respath,resdesc,imgurl,keywords from sysdb_resources r where r.resid=:resid and restype='map' and r.flag>=0
